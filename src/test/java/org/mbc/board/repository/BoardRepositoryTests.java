...
public class BoardRepositoryTests {
    ...
    public void testInsert() {
        ...
        IntStream.rangeClosed(1, 100).forEach(i -> {
                    ...
                    Board board = Board.builder()
                            ...
                            .content("내용" + i)
                            .writer("user" + (i % 10))
                            .build(); // @Builder용 (세터 대신 좀더 간단하고 가독성 좋게)
                    // log.info((board));
                    Board result = boardRepository.save(board); // 데이터베이스에 기록하는 코드
                    //                            .save 메서드는 jpa에서 상속한 메서드로 값을저장 하는 용도로 사용
                    //                              이미 값이 있으면 update를 진행한다.
                    log.info("게시물번호 출력 : " + result.getBno() + "게시물의 제목 : " + result.getTitle());
                } // forEach문 종료
    ...
    public void testSelect(){
        ...
        //                                  .findById(bno)
        Board board = result.orElseThrow(); // 값이 없으면 넣어라

        log.info(bno + "개의 데이터베이스에 존재합니다");
        log.info(board); // Board(bno=100, title=제목100, content=내용100, writer=user0)
    ...
    public void testUpdate(){
        ...
        Long bno = 100L; // 100번 게시물을 가져와서 수정 후 테스트종료
        ...
        board.change("수정 테스트 제목", "수정 테스트 내용"); // 제목과 내용만 수정할수 있는 메서드
    ...
    public void testPaging(){
        // .FindaAll() 는 모든 리스트를 출력하는 메서드 select * from board;
        ...
        //Hibernate:
        //    select
        //        count(b1_0.bno) board 전체 리스트 수를 알아옴.(쪼개기위함)
        //    from
        //        board b1_0
        // 1장에 종이에 Board 객체를 가지고 있는 결과는 result 에 담긴다
        // Page 클래스는 다음 페이지에 존재여부, 이전페이지 존재여부, 전체 데이터 개수, 등등 계산한다
